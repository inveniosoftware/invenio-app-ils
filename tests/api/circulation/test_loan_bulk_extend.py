...
pid_start_value = 99
amount_of_ongoing_loans = 5
pid_end_value = pid_start_value + amount_of_ongoing_loans
...
def create_loans(start=pid_start_value, end=pid_end_value):
    ...
    now = arrow.utcnow()
    end_date_not_expired = now + timedelta(days=day_range_loan_extendable - 1)

    def loan_generator(pid_value):
        loan = {
            "document_pid": "docid-1",
            "end_date": end_date_not_expired.isoformat(),
            "item_pid": {
                "type": "pitmid",
...
def test_loan_extend_permissions(
    ...
    def _run_asserts(extended_loans, ongoing_loans, patron_pid="1"):

        assert len(extended_loans) >= len(ongoing_loans)

        for extended_loan in extended_loans:
            for ongoing_loan in ongoing_loans:
                if ongoing_loan['pid'] == extended_loan['pid']:
                    assert extended_loan['state'] == "ITEM_ON_LOAN"
                    assert extended_loan['patron_pid'] == str(patron_pid)
                    assert extended_loan['end_date'] > ongoing_loan['end_date']
