...
def _fetch_loan(app, json_headers, loan, user=None):
...
def _assert_get_loan_success(app, json_headers, loan, user):
    """Assert that GET API call succeeded."""
    res = _fetch_loan(app, json_headers, loan=loan, user=user)
...
def test_anonymous_cannot_get_any_loan(app, json_headers, testdata):
    ...
    res = _fetch_loan(app, json_headers, loan=loan_patron1)
    assert res.status_code == 401
    res = _fetch_loan(app, json_headers, loan=loan_patron2)
    assert res.status_code == 401


def test_admin_or_librarian_can_get_any_loan(app, json_headers, users,
                                             testdata):
    ...
    for user in [users['admin'], users['librarian']]:
        ...
        loan_patron1 = testdata['loans'][0]
        assert loan_patron1['patron_pid'] == "1"
        _assert_get_loan_success(app, json_headers, loan=loan_patron1,
                                 user=user)

        # GET loans of Patron 2
        loan_patron2 = testdata['loans'][3]
        assert loan_patron2['patron_pid'] == "2"
        _assert_get_loan_success(app, json_headers, loan=loan_patron2,
                                 user=user)


def test_patron_can_get_only_his_loans(app, json_headers, users, testdata):
    ...
    res = _fetch_loan(app, json_headers, loan=loan_patron2, user=patron1)
    ...
    res = _fetch_loan(app, json_headers, loan=loan_patron1, user=patron2)
    assert res.status_code == 403
...
def test_anonymous_cannot_update_loans(app, json_headers, json_patch_headers,
                                       testdata):
...
def test_admin_or_librarian_can_update_loans(app, json_headers,
...
def test_patron_cannot_update_loans(app, json_headers, json_patch_headers,
                                    users, testdata):
