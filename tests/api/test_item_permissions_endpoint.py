...
def user_login(user_id, client, users):
    ...
    if user_id != 'anonymous':
        login_user_via_session(client,
                               email=User.query.get(users[user_id].id).email)
...
@pytest.mark.parametrize('user_id,res_id,expected_resp_code',
                         [
...
def test_get_item_endpoint(client, json_headers, testdata, users, user_id,
                           res_id, expected_resp_code):
...
def test_post_item_endpoint(client, json_headers, testdata, users, user_id,
                            res_id, expected_resp_code):
    ...
    if 'item_pid' in ITEM:
        del ITEM['item_pid']
    res = _test_response(client, 'post', url, json_headers, ITEM,
                         expected_resp_code)
...
def test_put_item_endpoint(client, json_headers, testdata, users, user_id,
                           res_id, expected_resp_code):
    """Test PUT permissions of an item."""
    url = url_for('invenio_records_rest.itemid_item', pid_value=res_id)
    user_login(user_id, client, users)
    res = _test_response(client, 'put', url, json_headers, NEW_ITEM,
                         expected_resp_code)
...
def test_patch_item_endpoint(client, json_patch_headers, testdata, users,
    ...
    res = _test_response(client, 'patch', url, json_patch_headers, data=patch,
...
def test_delete_item_endpoint(client, json_headers, testdata, users,
                              user_id, res_id, expected_resp_code):
    """Test DELETE permissions of an item."""
    user_login(user_id, client, users)
    url = url_for('invenio_records_rest.itemid_item', pid_value=res_id)
    res = _test_response(client, 'delete', url, json_headers, data=NEW_ITEM,
                         expected_resp_code=expected_resp_code)
